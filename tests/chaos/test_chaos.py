"""Chaos tests — simulated infrastructure failures during DAG execution.

Tests:
  1. Kill PostgreSQL mid-flow (verify task status recovery)
  2. Kafka publish failure (verify task stays PENDING)
  3. Simulate slow AI service (verify timeout/retry handling)
...
class TestPostgresFailureMidFlow:
    """Simulate PostgreSQL dying during DAG execution."""

    def test_db_dies_during_status_update(self):
        """Create task, execute DAG, DB dies when updating to COMPLETED."""
        task = create_task({"text": "Hello world"}, ["ner_result"])
        task_id = task["id"]

        plan = build_plan({"text": "Hello world"}, ["ner_result"])
        result = run_plan(plan, {"text": "Hello world"}, task_id)
        ...
        _write_report("test_db_dies_during_status_update", [
            f"Task {task_id}: DAG executed successfully",
    ...
    def test_db_dies_during_task_creation(self):
        """DB failure during task creation propagates cleanly."""
        with patch("src.services.task_service.get_session") as mock_session:
            mock_sess = MagicMock()
...
class TestKafkaFailureMidFlow:
    ...
    def test_kafka_publish_failure_task_stays_pending(self):
        ...
        _write_report("test_kafka_publish_failure_task_stays_pending", [
            ...
            "Kafka publish failed -> task stays PENDING (recoverable)",
            "PASS",
        ])

    def test_kafka_output_failure_task_still_completed_in_db(self):
        """DAG completes but Kafka output publish fails — DB should still be COMPLETED."""
        task = create_task({"text": "hello"}, ["ner_result"])
        task_id = task["id"]

        plan = build_plan({"text": "hello"}, ["ner_result"])
        result = run_plan(plan, {"text": "hello"}, task_id)
        update_task_status(task_id, "COMPLETED", final_output=result)
        ...
        _write_report("test_kafka_output_failure_task_still_completed_in_db", [
            f"Task {task_id}: DAG executed, DB updated to COMPLETED",
...
class TestSlowAIService:
    ...
    def test_http_timeout_propagates(self):
        ...
        _write_report("test_http_timeout_propagates", [
            "Simulated: HTTP request timeout",
...
class TestCircuitBreakerTrip:
    """Simulate circuit breaker trip during DAG execution."""

    def test_breaker_trips_mid_flow(self):
        """Trip the circuit breaker, then verify it blocks requests."""
        ...
        _write_report("test_breaker_trips_mid_flow", [
